            ...
            {!! Form::open(['action' => 'WorksController@store', 'method' => 'POST']) !!}
                ...
                <div class="form-group">
                    <div class="row">{{Form::label('description', 'Description', ['class' => 'col-md-3'])}}</div>
                        {{Form::textarea('description','' , ['class' => 'form-control autosize-target text-left','rows' => '5' , 'placeholder' => 'Description'])}}
                </div>
                <div class="form-group">
                    <div class="row">{{Form::label('days', 'Days', ['class' => 'col-md-3'])}}</div>
                    {{Form::number('days', '', ['class' => 'form-control', 'placeholder' => 'Enter nbr days'])}}  
                </div>
                <div class="form-group">
                    <div class="row">
                        {{Form::label('company_id', 'Company', ['class' => 'col-md-3'])}}
                        <div class="col-md-6">{{Form::select('company_id', array_pluck( $company , 'name','id'), 'Choose Company', ['class' => 'form-control', 'placeholder' => 'Choose Category']) }}</div>       
                    </div>
                </div>
                <div class="form-group">
